...
def parse_section(label, number=2, number_parser=floatNumber):
    return pa.Suppress(pa.SkipTo(label)) + \
        pa.Suppress(pa.SkipTo(number_parser)) + number_parser * number
...
def generate_parser():
    ...
    parser_section = parser_step + parser_pressure + \
        parser_volumen + parser_cell_lnth + parser_cell_angl
